...
**PhantomHunter** and the training process. Given an input sample $\mathbf{x}$ and a same/different-family sample $\tilde{\mathbf{x}}$, it **1)** performs feature extraction with two probability-list branches: a lightweight RoBERTa-MLP proxy branch predicts proxy probability lists $\hat{P}_M$, while the base LLM branch extracts base probability lists $P_M$ as training supervision. The proxy branch is optimized with $\mathcal{L}_{proxy}$ to approximate the base probability lists, and the base LLM branch is dropped during inference for efficient deployment. The selected probability representation is then encoded by CNN and Transformer blocks; **2)** a family predictor produces family predictions for $\mathbf{x}$ and $\tilde{\mathbf{x}}$, while contrastive family-aware learning $\mathcal{L}_C$ encourages samples from the same family to share closer representations; and **3)** a gate derived from the family predictor controls a mixture-of-experts detector, whose family-specific detectors are combined to produce the final LLM-generated text prediction. The red terms are loss functions.
